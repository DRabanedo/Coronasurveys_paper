...
hp_df = gen_SIRpop(N, net_model ,beta = 0.04, gamma = 0.015, chosen_nodes = 1, n_iter = 5)
...
# Study parameters
parameters = round(seq(from = 1, to = 100, length.out = 15))
...
################################################################################
file_name = str_c("Simulation_networkneighbours_notdisjoint_sir_sw_", seed,".csv")
...
################################################################################
file_name_disjoint = str_c("Simulation_networkneighbours_disjoint_sir_sw_", seed,".csv")
